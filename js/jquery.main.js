...
function initCarousel() {
	jQuery('.carousel:not(.slick)').scrollGallery({
		mask: '.mask',
		slider: '.slideset',
		slides: '.slide',
		btnPrev: 'a.btn-prev',
...
;(function($) {
	...
	GoogleMap.prototype = {
		...
		drawMarkers: function() {
			...
			this.addressObj.forEach(function(item, index) {
				var latLng = new google.maps.LatLng(item.address[0], item.address[1]);
				self.setMarker(latLng, item.info);
				self.centeringMap(latLng, index);
			});
		},
		setMarker: function(position, obj) {
			var popup = $(tmpl(this.holder.data('html').replace(/#+/g, ''), obj));

			var marker = new CustomMarker({
				latlng: position,
				map: this.map,
				pinHolder: popup && popup.length ? popup[0] : $('<div class="google-marker-holder"><a href="#" class="pin"><span class="icon-location"></span> <span class="icon-hat"></span></a></div>')[0]
			});

			new MapPopup({
				holder: popup,
				map: this.map,
				mapHolder: this.holder
			});
		},
		centeringMap: function(position, index) {
...
;(function($, window) {
	...
	function CustomMarker(options) {
		...
		this.pinHolder = this.options.pinHolder;
	}
	CustomMarker.prototype = new google.maps.OverlayView();

	CustomMarker.prototype.draw = function() {
		var panes = this.getPanes();
		panes.overlayImage.appendChild(this.pinHolder);

		var point = this.getProjection().fromLatLngToDivPixel(this.options.latlng);

		if (point) {
			this.pinHolder.style.left = point.x + 'px';
			this.pinHolder.style.top = point.y + 'px';
		}
	};

	CustomMarker.prototype.remove = function() {
		if (this.pinHolder) {
			this.pinHolder.parentNode.removeChild(this.pinHolder);
			this.pinHolder = null;
		}
	};
...
;(function($, window) {
	...
	MapPopup.prototype = {
		init: function() {
			if (this.options.holder && this.options.map && window.google) {
				this.findElements();
				this.attachEvents();
			}
		},
		findElements: function() {
			...
			this.popup = this.holder.find(this.options.popup).hide();
		...
		attachEvents: function() {
			...
			this.outsideClickHandler = function(e) {
				...
				if (!target.closest(self.holder).length && !target.is(self.holder)) {
					self.hidePopup();
				}
			};

			if (isWinPhoneDevice) {
				this.opener.on('pointerdown MSPointerDown', this.toggleHandler);
				this.opener.on('click', function(e) {
					e.preventDefault();
				});
			} else {
				this.opener.on('click', this.toggleHandler);
			}
		},
		showPopup: function() {
			var self = this;
		...
		checkPopupPosition: function() {
			var popupOffsets = {
				...
				vDiff = 0,
				hDiff = 0,
				topLimit = popupOffsets.top - holderOffsets.top,
				bottomLimit = (holderOffsets.top + holderOffsets.height) - popupOffsets.top - popupOffsets.height,
				leftLimit = popupOffsets.left - holderOffsets.left,
				rightLimit = (holderOffsets.left + holderOffsets.width) - popupOffsets.left - popupOffsets.width;
			...
			if (bottomLimit < 0) {
				vDiff = Math.abs(bottomLimit);
			}

			if (leftLimit < 0) {
				hDiff = Math.abs(leftLimit);
			}
		...
		destroy: function() {
			doc.off('click', this.outsideClickHandler);
			this.opener.off('click pointerdown MSPointerDown');
		}
	};
